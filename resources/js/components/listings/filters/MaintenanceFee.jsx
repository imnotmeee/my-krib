...
import { setFeeUri, clearFeeUri } from "../../../store/reducers/uri_slice";
...
export default function MaintenanceFee() {
    ...
    const handleSet = () => {
        if (val === 0) {
            setPreVal(0);
            setActive(false);
            dispatch(clearFeeUri());
            handleClose();
            return;
        }
        setPreVal(val);
        setActive(true);
        dispatch(setFeeUri(`maxMaintenanceFee=${val}&`));
        handleClose();
    };
    const handleClear = () => {
        setPreVal(0);
    ...
    return (
        <>
            ...
            <Dialog open={open} onClose={handleClose}>
                <DialogTitle>Maintenance Fee</DialogTitle>
                <DialogContent>
                    <Box width={300} py={5}>
                        <Slider
                            value={val}
